...
**Dialog / picker plumbing goes through `ReactiveUI.Interaction`**, not code-behind. VMs declare `public Interaction<TRequest, TResponse> Foo { get; } = new();` and call `await Foo.Handle(...)`. Views register handlers in `OnAttachedToVisualTree` via `QCEDL.GUI/Services/DialogInteractions.cs::DialogBridges.{RegisterPickFile,RegisterSaveFile,RegisterPickFolder,RegisterConfirm}`, stash the `IDisposable` in a `CompositeDisposable`, and clear it in `OnDetachedFromVisualTree`. Request DTOs (`OpenFileRequest`, `SaveFileRequest`, `OpenFolderRequest`, `ConfirmRequest`) and shared `FilePickerTypes` live in the same file. Never resolve `TopLevel.GetTopLevel(this)` / `StorageProvider` from a VM — that's the view's job via the bridges.
