...
- `QCEDL.NET/` — protocol + transport + shared orchestration library. `Core/` hosts the **public** orchestration surface (`EdlManager`, `EdlOptions`, `DeviceMode`, `StorageGeometry`, `BlockDeviceManagerBase`, `HostDeviceManager`, `RadxaWoSDeviceManager`) **plus the higher-level `*Runner` flows** (`RawProgramRunner`, `DumpRawprogramRunner`, `ProvisionRunner`). `Helpers/` holds the shared `Logging` sink, `ProgressReporter`, `AlignmentHelper`, and `ImageSizeParser`. `Transport/Elevation/` hosts the launcher abstractions used by the privileged helper path (`ElevationPolicy`, `HelperLauncher`, `MacOsAscriptLauncher`, `MacSmAppServiceLauncher`). No UI or `System.CommandLine` dependencies. Exposes internals to `edl-ng`, `qcedl-gui`, and `edl-ng-helper` via `InternalsVisibleTo`. `AnyCPU` only.
- `QCEDL.GUI/` — Avalonia 12 + ReactiveUI desktop app. References `QCEDL.NET` directly and drives `EdlManager` — it does **not** shell out to the `edl-ng` executable and does **not** depend on `QCEDL.CLI`. Publishes `edl-ng-helper` alongside `qcedl-gui` via a content-only `<ProjectReference>` so `HelperLauncher.ResolveHelperPath` finds it at runtime. `AnyCPU` only, self-contained publish to match CLI's `NETSDK1151` constraint.
- `QCEDL.Helper/` — standalone `edl-ng-helper` console executable. Invoked by `MacOsAscriptLauncher` (and future `MacSmAppServiceLauncher`) under administrator privileges; connects back to the GUI via a Unix-domain socket passed as `--socket <path>`. References `QCEDL.NET` so it reuses the native `QualcommSerial` backends. See the "Elevation / privileged device access" section below for the protocol details.
